    ...
    companion object{
        ...
        fun serialize(lobby: Lobby): SerializedLobby{
            with(lobby){
                ...
                return SerializedLobby(id.toString(), code, name, timeCreated,
                    timeUpdated, userList.toList(), status.toString(), showResults)
            }
        }
        ...
        suspend fun deSerialize(context: Context, serializedLobby: SerializedLobby): Lobby{
            with(serializedLobby){
                ...
                val userList: List<User> =
                    LobbyRepository.getInstance(context).loadUsersWithString(context, null, serializedLobby.users)
        ...
        suspend fun loadSnapshot(context: Context, oldUserList: List<User>?, snapshot: DocumentSnapshot, shouldLoadAvatar: Boolean = false): Lobby{
            ...
            val lobbyState: LobbyState = LobbyState.valueOf(status)
            val userList: List<User> =
                LobbyRepository.getInstance(context).loadUsersWithString(context, oldUserList, stringList, shouldLoadAvatar)
